...
public:
    ...
    bool WriteZPSCSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPSCSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPSCSSeed_deprecated(uint256& seed);
    bool EraseZPSCSSeed();
    bool EraseZPSCSSeed_deprecated();

    bool WriteZPSCSCount(const uint32_t& nCount);
    bool ReadZPSCSCount(uint32_t& nCount);
