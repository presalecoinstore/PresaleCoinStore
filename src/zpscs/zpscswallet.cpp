...
CzPSCSWallet::CzPSCSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpscs seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPSCSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPSCSSeed_deprecated()) {
                    LogPrintf("%s: Updated zPSCS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpscs seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpscs wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPSCSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPSCSCount(nCountLastUsed);
    else if (!walletdb.ReadZPSCSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPSCSWallet::Lock()
{
    seedMaster = 0;
}

void CzPSCSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPSCSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPSCS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPSCSWallet::LoadMintPoolFromDB()
{
...
void CzPSCSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPSCSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPSCSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpscsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPSCSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPSCS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpscsTracker which also adds to database
    pwalletMain->zpscsTracker->Add(dMint, true);
...
void CzPSCSWallet::SeedToZPSCS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPSCSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPSCSWallet::GenerateDeterministicZPSCS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPSCSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPSCSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPSCSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
