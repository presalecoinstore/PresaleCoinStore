// Copyright (c) 2018-2019 The PreSaleCoinStore developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zpscs/deterministicmint.h>
#include "zpscstracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzPSCSTracker::CzPSCSTracker(std::string strWalletFile)
{
...
CzPSCSTracker::~CzPSCSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPSCSTracker::Init()
{
...
bool CzPSCSTracker::Archive(CMintMeta& meta)
{
...
bool CzPSCSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPSCSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPSCSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPSCSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPSCSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPSCSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPSCSTracker::GetSerialHashes()
{
...
CAmount CzPSCSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPSCSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPSCSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPSCSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPSCSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPSCSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPSCSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPSCSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPSCSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPSCSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPSCSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPSCSWallet* zPSCSWallet)
{
    bool iszPSCSWalletInitialized = (NULL != zPSCSWallet);
    CMintMeta meta;
    ...
    if (! iszPSCSWalletInitialized)
        zPSCSWallet = new CzPSCSWallet(strWalletFile);
    meta.isSeedCorrect = zPSCSWallet->CheckSeed(dMint);
    if (! iszPSCSWalletInitialized)
        delete zPSCSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPSCSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPSCSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPSCSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPSCSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPSCSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPSCSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPSCSWallet* zPSCSWallet = new CzPSCSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zPSCSWallet);
        }
        delete zPSCSWallet;
        LogPrint("zero", "%s: added %d dzpscs from DB\n", __func__, listDeterministicDB.size());
    }
