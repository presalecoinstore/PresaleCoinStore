...
// The zPSCS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPSCS is a hash of the serial
...
{
    //Create an output returning the zPSCS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPSCSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPSCS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPSCS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPSCSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPSCS output", __func__);
...
{
    CzPSCSTracker* zpscsTracker = pwallet->zpscsTracker.get();
    CMintMeta meta;
    if (!zpscsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpscsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PSCS stake is the outpoint
