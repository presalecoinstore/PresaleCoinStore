...
#include "ui_zpscscontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zpscsTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpscsTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zPSCS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPSCS seed used to mint this zPSCS is not the same as currently hold in the wallet";
            else
