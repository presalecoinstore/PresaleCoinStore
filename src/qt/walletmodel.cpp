...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered presalecoinstore address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 PSCS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal presalecoinstore:URI (presalecoinstore:XyZ...?message=example)
            {
...
static void NotifyzPSCSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzPSCSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzPSCSReset.connect(boost::bind(NotifyzPSCSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzPSCSReset.disconnect(boost::bind(NotifyzPSCSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zpscsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
